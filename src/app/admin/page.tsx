...
export default async function AdminDashboard({
  ...
  searchParams: Promise<{ page?: string; status?: string; cp?: string }>
}) {
  ...
  const companyPage = parseInt(searchParams.cp || "1")
  const limit = 10
  const skip = (page - 1) * limit
  const companySkip = (companyPage - 1) * limit
  const statusFilter = searchParams.status

  const [pendingCompanies, companiesData, dsarData, stats] = await Promise.all([
  ...
  const totalCompanyPages = Math.ceil(companiesData.count / limit)
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 gap-12">
          ...
          <section>
                ...
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                  {companiesData.companies.map((company) => (
                    <tr key={company.id} className="transition-colors hover:bg-zinc-50/50 dark:hover:bg-zinc-800/20">
                      ...
                      <td className="px-6 py-5">
                        <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-tight ${company.status === 'APPROVED' ? 'border-emerald-100 bg-emerald-50 text-emerald-600 dark:border-emerald-900/30 dark:bg-emerald-900/20' :
                      ...
                      <td className="px-6 py-5 text-right">
                        {company.status !== "PENDING" && (
                          <form action={handleCompanyUpdate}>
                            <input type="hidden" name="companyId" value={company.id} />
                            <input type="hidden" name="status" value={company.status === "APPROVED" ? "INACTIVE" : "APPROVED"} />
                            <button className={`rounded-lg px-3 py-1.5 text-[10px] font-black uppercase tracking-widest transition-all ${company.status === "APPROVED"
                              ? "bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400"
                              : "bg-emerald-50 text-emerald-600 hover:bg-emerald-100 dark:bg-emerald-900/20 dark:text-emerald-400"
                              }`}>
                              {company.status === "APPROVED" ? "Deactivate" : "Activate"}
                            </button>
                          </form>
                        )}
                      </td>
            ...
            {totalCompanyPages > 1 && (
              <div className="mt-8 flex items-center justify-between border-t border-zinc-100 pt-6 dark:border-zinc-800">
                <p className="text-xs font-bold text-zinc-400">Page {companyPage} of {totalCompanyPages}</p>
                <div className="flex gap-2">
                  {companyPage > 1 && (
                    <Link href={`/admin?cp=${companyPage - 1}${page > 1 ? `&page=${page}` : ''}${statusFilter ? `&status=${statusFilter}` : ''}`} className="rounded-xl border border-zinc-200 px-4 py-2 text-xs font-bold transition-all hover:bg-zinc-50 dark:border-zinc-700 dark:hover:bg-zinc-800">
                      Previous
                    </Link>
                  )}
                  {companyPage < totalCompanyPages && (
                    <Link href={`/admin?cp=${companyPage + 1}${page > 1 ? `&page=${page}` : ''}${statusFilter ? `&status=${statusFilter}` : ''}`} className="rounded-xl bg-emerald-950 px-4 py-2 text-xs font-bold text-white transition-all hover:bg-emerald-900 dark:bg-emerald-800">
                      Next
                    </Link>
                  )}
                </div>
